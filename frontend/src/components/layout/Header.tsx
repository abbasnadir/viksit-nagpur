      ...
      <header className="flex-shrink-0 bg-navy-900 text-white border-b border-navy-800" style={{ height: '56px' }}>
        <div className="flex items-center h-full px-4">
          ...
          <button
            ...
            className="flex items-center gap-2 text-xs text-navy-200 hover:text-white px-2 py-1.5 rounded hover:bg-navy-800 transition-colors cursor-pointer"
          >
            <div className="w-7 h-7 rounded-full bg-navy-700 border border-navy-600 flex items-center justify-center text-xs font-bold text-amber-300">
              AD
            </div>
            <span className="font-semibold text-slate-100">{state.operatorName}</span>
            <ChevronDown size={12} className="text-navy-400" />
          </button>
        </div>
      </header>

      {/* Profile & Console Settings Modal */}
